...
func TestExtractTrailingPunctuation_OpenBrace(t *testing.T) {
	punct, rest := extractTrailingPunctuation("func foo() {")
	if punct != "{" {
		t.Errorf("expected punct '{', got %q", punct)
	}
	if rest != "func foo() " {
		t.Errorf("expected rest 'func foo() ', got %q", rest)
	}
}
...
func TestExtractTrailingPunctuation_SemicolonInStringNotExtracted(t *testing.T) {
	punct, rest := extractTrailingPunctuation(`print("a; b")`)
	if punct != "" {
		t.Errorf("expected punct '', got %q", punct)
	}
	if rest != `print("a; b")` {
		t.Errorf("expected rest 'print(\"a; b\")', got %q", rest)
	}
}

func TestExtractTrailingPunctuation_BraceInStringNotExtracted(t *testing.T) {
	punct, rest := extractTrailingPunctuation(`print("}");`)
	if punct != ";" {
		t.Errorf("expected punct ';', got %q", punct)
	}
	if rest != `print("}")` {
		t.Errorf("expected rest 'print(\"}\")', got %q", rest)
	}
}
...
func TestExtractTrailingPunctuation_PunctWithTrailingComment(t *testing.T) {
	punct, rest := extractTrailingPunctuation("let x = 1; // trailing comment")
	if punct != ";" {
		t.Errorf("expected punct ';', got %q", punct)
	}
	if rest != "let x = 1" {
...
func TestExtractTrailingPunctuation_PunctWithTrailingBlockComment(t *testing.T) {
	punct, rest := extractTrailingPunctuation("let x = 1; /* trailing */")
	if punct != ";" {
		t.Errorf("expected punct ';', got %q", punct)
	}
	if rest != "let x = 1" {
...
func TestExtractTrailingPunctuation_BacktickIdentifier(t *testing.T) {
	// Swift uses backticks to escape identifiers
	punct, rest := extractTrailingPunctuation("let `type` = 1;")
	if punct != ";" {
		t.Errorf("expected punct ';', got %q", punct)
	}
	if rest != "let `type` = 1" {
		t.Errorf("expected rest 'let `type` = 1', got %q", rest)
	}
}
